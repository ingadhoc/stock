...
class StockRequestAbstract(models.AbstractModel):
    ...
    @api.constrains("company_id", "product_id", "warehouse_id", "location_id", "route_id")
    def _check_company_constrains(self):
        ...
        for rec in self:
            ...
            if rec.location_id.company_id and rec.location_id.company_id != rec.company_id:
                raise ValidationError(self.env._("You have entered a location that is " "assigned to another company."))
            if rec.warehouse_id.company_id != rec.company_id:
                raise ValidationError(
                    self.env._("You have entered a warehouse that is " "assigned to another company.")
                )
            if rec.route_id and rec.route_id.company_id and rec.route_id.company_id != rec.company_id:
                raise ValidationError(self.env._("You have entered a route that is " "assigned to another company."))

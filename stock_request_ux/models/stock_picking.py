...
class StockPicking(models.Model):
    ...
    def _compute_stock_request_order_ids(self):
        for rec in self:
            rec.stock_request_order_ids = rec.move_ids.mapped("stock_request_ids.order_id")
            rec.stock_request_order_count = len(rec.stock_request_order_ids)

    def action_view_stock_order_request(self):
        """
        :return dict: dictionary value for created view
        """
        action = self.env.ref("stock_request.stock_request_order_action").sudo().read()[0]

        request_orders = self.mapped("stock_request_order_ids")
        if len(request_orders) > 1:
            action["domain"] = [("id", "in", request_orders.ids)]
        elif request_orders:
            action["views"] = [(self.env.ref("stock_request.stock_request_order_form").id, "form")]

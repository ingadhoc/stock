...
class ProductReplenish(models.TransientModel):
    ...
    This is a backport from odoo v12, replaces the "request procurement"
     wizard from v10, lost during the stock refactoing.
    TODO remove in v12
    """
    ...
    product_has_variants = fields.Boolean(
        'Has variants', default=False, required=True)
    product_uom_category_id = fields.Many2one(
        'product.uom.categ', related='product_id.uom_id.category_id',
        readonly=True, required=True)
    product_uom_id = fields.Many2one(
        'product.uom', string='Unity of measure', required=True)
    ...
    route_ids = fields.Many2many(
        ...
        help="Apply specific route(s) for the replenishment"
        " instead of product's default routes.")

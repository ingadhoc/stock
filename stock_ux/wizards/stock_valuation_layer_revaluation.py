...
class StockValuationLayerRevaluation(models.TransientModel):
    ...
    def action_validate_revaluation(self):
        ...
        product_id = self.product_id.with_context(force_company=self.company_id.id)
        ...
        if product_id.categ_id.property_cost_method == 'average':
            description += _(
                " Product cost updated from %s to %s." % 
                (product_id.standard_price,
                product_id.standard_price + self.added_value / self.current_quantity_svl
            ))
        ...
        move_vals = {
            ...
            'type': 'entry',
            'line_ids': [(0, 0, {
                'name': _('%s changed stock valuation from  %s to %s - %s' % (
                    self.env.user.name,
                    self.current_value_svl,
                    self.current_value_svl + self.added_value,
